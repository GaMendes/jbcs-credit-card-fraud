...
class ModelWrapper:
    def __init__(self, model, params):
        """
        Initializes the class with the classification model and its parameters.

        Args:
          model: The scikit-learn classification model.
          params: A dictionary with the hyperparameters to be optimized.
        """
        self.model              = model
        self.params             = params
    ...
    def _optimize_hyperparameters(self, X, y):
        ...
        skf = RepeatedStratifiedKFold(n_splits=3, n_repeats=1, random_state=1)
        grid = GridSearchCV(self.model, self.params, cv=skf, n_jobs=-1)
        grid.fit(X, y)
    ...
    def train(self, X, y, dataset):
        ...
        for train_index, test_index in skf.split(X, y):
            ...
            self.balanced_acuracies.append(balanced_accuracy_score(y_test, y_pred))
    ...
    def explain_shap(self, X):
        ...
        shap_explainer = shap.Explainer(self.model.predict, X, seed=1)
    ...
    def get_results(self):
        ...
        return {
            ...
            'balanced_acuracies': self.balanced_acuracies,
